...
try:
    ...
    def train_face_model(status_callback=None):
        return False, f"Training module unavailable: {str(e)}"

    def get_training_status():
        return {"training": False, "message": "Training module unavailable", "progress": 0}



app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "supersecretkey_studentfacial_2026")
...
def load_face_cascade():
    ...
    candidates = [
        os.path.join(BASE_DIR, cascade_filename),
        os.path.join(getattr(cv2, 'data', None) and getattr(cv2.data, 'haarcascades', '') or '', cascade_filename),
    ...
    # Default attempt
    default_p = cv2.data.haarcascades + cascade_filename
    casc = cv2.CascadeClassifier(default_p)
    return casc, default_p

FACE_CASCADE, cascade_path = load_face_cascade()
if FACE_CASCADE.empty():
    print(f"WARNING: Could not load Haar Cascade file from candidate locations. Path attempted: {cascade_path}")
...
@app.route("/logout")
def logout():
    global attendance_running, capture_running
    attendance_running = False
    capture_running = False
    release_camera()
    session.clear()
...
def start_capture(student_id):
    global capture_running, capture_student_id, capture_count, capture_done, capture_message
...
def generate_capture_frames():
    global camera, capture_running, capture_student_id
    global capture_count, capture_max_samples, capture_done, capture_message

    if capture_student_id is None:
        return

    student_folder = os.path.join(DATASET_DIR, str(capture_student_id))
    os.makedirs(student_folder, exist_ok=True)
    ...
    if not camera.isOpened():
        capture_running = False
        capture_message = "Could not access webcam."
        return
    ...
    while capture_running:
        ...
        for (x, y, w, h) in faces:
            if capture_count < capture_max_samples:
                ...
                save_path = os.path.join(student_folder, f"{capture_count}.jpg")
        ...
        if capture_count >= capture_max_samples:
            capture_done = True
            capture_running = False
            capture_message = f"Face captured successfully. {capture_count} images saved."
            break
...
@app.route("/stop_capture_and_train")
@login_required
def stop_capture_and_train():
    global capture_running, capture_done, capture_message, training_thread
    ...
    training_thread = threading.Thread(target=background_train_model, daemon=True)
